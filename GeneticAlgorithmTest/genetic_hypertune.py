...
def genetic_hypertune_autoencoder(prefix_name = 'testmodel',
    ...
    gene_space = [[0.5,1.0,1.5,2.0,2.5],list(range(10,200,20)),[0.001,0.0005],[8,16,32,64],[1,2]],
    ...
    def save_callback(ga_instance):
        ...
        filename = f"{savedir}/ga_instance_generation_{ga_instance.generations_completed}.pkl"
    ...
    ga_instance = pygad.GA(num_generations=ga_settings.pop('num_generations'),
                        ...
                        save_solutions=True,
                        callback_generation=save_callback,
...
def read_past_ga(filename='tmp/ga_instance'):
    ...
    '''
    ga_instance = dill.load(open(filename, 'rb'))
    print("Best solutions : ", ga_instance.best_solutions)
    print("Best solutions fitness : ", ga_instance.best_solutions_fitness)
    # just print all solutions with a for loop with solutions and solutions_fitness
    for i in range(len(ga_instance.solutions)):
...
def continue_run_ga(filename='tmp/ga_instance'):
    '''
    continue run ga instance
    Parameters
    ----------
    filename : str, optional. The default is 'tmp/ga_instance'.
    '''
    ga_instance = dill.load(open(filename, 'rb'))
    ga_instance.run()
    ga_instance.plot_result(title="PyGAD & PyPlot - Iteration vs. Fitness", linewidth=4, save_dir='./tmp/')
    
    filename = f"ga_instance_generation_{ga_instance.generations_completed}.pkl"
    with open(filename, "wb") as f:
        dill.dump(ga_instance, f)
    print(f"Saved GA instance to file: {filename}")
